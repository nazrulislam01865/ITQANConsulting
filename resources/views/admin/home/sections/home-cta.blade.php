...
<div class="form-grid">
  <div class="field full">
    ...
    <div class="image-preview" style="max-width:280px;background:#08111f;padding:18px;border-radius:24px">
      <img src="{{ $qrPreviewUrl }}" alt="Current digital contact QR code" style="width:100%;aspect-ratio:1;object-fit:contain">
    </div>
  ...
  <div class="field"><label>Eyebrow text</label><input name="label" value="{{ old('label', $section->label) }}"></div>
  <div class="field full"><label>Headline</label><textarea name="title">{{ old('title', $section->title) }}</textarea></div>
  <div class="field full"><label>Description</label><textarea name="lead">{{ old('lead', $section->lead) }}</textarea></div>
  ...
  <div class="field"><label>Downloaded contact filename</label><input name="settings[contact_file_name]" value="{{ old('settings.contact_file_name', array_key_exists('contact_file_name', $settings) ? $settings['contact_file_name'] : ($defaults['contact_file_name'] ?? 'ITQAN-Consulting-Digital-Card.vcf')) }}"></div>
