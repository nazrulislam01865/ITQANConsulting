...
<main id="contact" class="page active">
  <?php echo $__env->make('frontend.partials.plain-hero', ['hero' => $page['hero']], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
  ...
  <section class="section contact-experience-section" id="contact-form" aria-labelledby="contactExperienceTitle">
    <div class="container">
      <div class="contact-experience-intro reveal">
        <div class="label"><?php echo e($form['label']); ?></div>
        <h2 id="contactExperienceTitle"><?php echo e($form['title']); ?></h2>
        <p><?php echo e($form['intro']); ?></p>
      </div>

      <div class="contact-experience-grid <?php echo e($ctaActive ? '' : 'single-column'); ?>">
        <form
          ...
          data-contact-wizard
          data-initial-step="<?php echo e($initialStep); ?>"
        >
          <?php echo csrf_field(); ?>

          <div class="contact-wizard-form" <?php if(session('status')): ?> hidden <?php endif; ?>>
            <?php if($errors->any()): ?>
            ...
            <div class="contact-wizard-step" data-wizard-step="0">
              <h3><?php echo e($wizardSteps[0]['title']); ?></h3>
              <p><?php echo e($wizardSteps[0]['text']); ?></p>
              <div class="contact-choice-grid" role="group" aria-label="Business problems">
                <?php $__currentLoopData = $problemOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <?php $optionKey = $normalizeContactOption($option); ?>
                  <label class="contact-choice-chip <?php echo e(in_array($optionKey, $selectedProblemKeys, true) ? 'selected' : ''); ?>">
                    <input type="checkbox" name="areas[]" value="<?php echo e($option); ?>" <?php if(in_array($optionKey, $selectedProblemKeys, true)): echo 'checked'; endif; ?>>
            ...
            <div class="contact-wizard-step" data-wizard-step="1" hidden>
              <h3><?php echo e($wizardSteps[1]['title']); ?></h3>
              <p><?php echo e($wizardSteps[1]['text']); ?></p>
              <div class="contact-choice-grid" role="group" aria-label="Support types">
                <?php $__currentLoopData = $supportOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <?php $optionKey = $normalizeContactOption($option); ?>
                  <label class="contact-choice-chip <?php echo e(in_array($optionKey, $selectedSupportKeys, true) ? 'selected' : ''); ?>">
                    <input type="checkbox" name="support_types[]" value="<?php echo e($option); ?>" <?php if(in_array($optionKey, $selectedSupportKeys, true)): echo 'checked'; endif; ?>>
                    <span><?php echo e($option); ?></span>
                  </label>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
              </div>
            ...
            <div class="contact-wizard-step" data-wizard-step="2" hidden>
              <h3><?php echo e($wizardSteps[2]['title']); ?></h3>
              <p><?php echo e($wizardSteps[2]['text']); ?></p>
              <div class="contact-wizard-fields">
                <div class="contact-wizard-field">
                  <label for="name">Name</label>
                  <input id="name" name="name" type="text" value="<?php echo e(old('name')); ?>" placeholder="Your name" autocomplete="name" required>
                </div>
                <div class="contact-wizard-field">
                  <label for="company">Company</label>
                  <input id="company" name="company" type="text" value="<?php echo e(old('company')); ?>" placeholder="Company or organization" autocomplete="organization">
                </div>
                <div class="contact-wizard-field">
                  <label for="email">Email</label>
                  <input id="email" name="email" type="email" value="<?php echo e(old('email')); ?>" placeholder="name@company.com" autocomplete="email" required>
                </div>
                <div class="contact-wizard-field">
                  <label for="phone">Phone / WhatsApp</label>
                  <input id="phone" name="phone" type="tel" value="<?php echo e(old('phone')); ?>" placeholder="Your number" autocomplete="tel">
                </div>
                <div class="contact-wizard-field full">
                  <label for="method">Preferred contact</label>
                  <select id="method" name="method">
                    <?php $__currentLoopData = $contactMethods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $method): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                      <option value="<?php echo e($method); ?>" <?php if(old('method') === $method): echo 'selected'; endif; ?>><?php echo e($method); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                  </select>
                </div>
              </div>
            ...
            <div class="contact-wizard-step" data-wizard-step="3" hidden>
              <h3><?php echo e($wizardSteps[3]['title']); ?></h3>
              <p><?php echo e($wizardSteps[3]['text']); ?></p>
              <div class="contact-wizard-fields">
                <div class="contact-wizard-field full">
                  <label for="message">What is happening, and what would you like to improve?</label>
                  <textarea id="message" name="message" placeholder="Tell us the messy version..." required><?php echo e(old('message')); ?></textarea>
                </div>
                <div class="contact-wizard-field full">
                  <label for="budget">Tentative budget range</label>
                  <select id="budget" name="budget">
                    <?php $__currentLoopData = $budgetOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $budget): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          ...
          <div class="contact-wizard-success <?php echo e(session('status') ? 'show' : ''); ?>" <?php if(!session('status')): ?> hidden <?php endif; ?> role="status">
            <div class="contact-wizard-success-icon" aria-hidden="true">✓</div>
            <h3><?php echo e($form['success_title']); ?></h3>
            <p><?php echo e(session('status') ?: $form['success_text']); ?></p>
            <a class="contact-wizard-button secondary" href="<?php echo e(route('contact')); ?>#contact-form">Send another message</a>
          </div>
        </form>
        ...
        <?php if($ctaActive): ?>
          <aside class="contact-qr-card reveal" aria-labelledby="contactQrTitle">
            <div>
              <div class="contact-qr-label"><?php echo e($cta['eyebrow'] ?? 'Save the contact'); ?></div>
              <h3 id="contactQrTitle"><?php echo e($cta['title'] ?? 'Keep ITQAN one scan away.'); ?></h3>
              <p><?php echo e($cta['text'] ?? 'Scan the QR code to save the founder’s contact details directly to your phone.'); ?></p>
            </div>

            <div class="contact-qr-frame" aria-label="<?php echo e($cta['qr_alt'] ?? 'ITQAN contact QR code'); ?>">
              <img src="<?php echo e($cta['qr_image_url']); ?>" alt="<?php echo e($cta['qr_alt'] ?? 'ITQAN contact QR code'); ?>" loading="lazy">
              <span class="contact-qr-scan-line" aria-hidden="true"></span>
            </div>

            <?php if(!empty($cta['qr_caption'])): ?>
              <p class="contact-qr-caption"><?php echo e($cta['qr_caption']); ?></p>
            <?php endif; ?>

            <div class="contact-direct-links">
              <?php if($phone !== ''): ?>
                <a href="tel:<?php echo e(preg_replace('/\s+/', '', $phone)); ?>"><span>Call</span><strong><?php echo e($phone); ?></strong></a>
              <?php endif; ?>
              <?php if($whatsapp !== ''): ?>
                <a href="https://wa.me/<?php echo e($whatsapp); ?>" target="_blank" rel="noopener noreferrer"><span>WhatsApp</span><strong>Open chat</strong></a>
              <?php endif; ?>
              <?php if($email !== ''): ?>
                <a href="mailto:<?php echo e($email); ?>"><span>Email</span><strong><?php echo e($email); ?></strong></a>
              <?php endif; ?>
            </div>

            <div class="contact-qr-actions">
              <button class="contact-qr-action" id="saveContactQrCardButton" type="button"><?php echo e($cta['save_button_text'] ?? 'Save Contact'); ?></button>
              <?php if($primaryHref): ?>
                <a class="contact-qr-action primary" href="<?php echo e($primaryHref); ?>" <?php if($primaryExternal): ?> target="_blank" rel="noopener noreferrer" <?php endif; ?>><?php echo e($cta['button']['text'] ?? 'Start a Conversation'); ?></a>
              <?php endif; ?>
            </div>
          </aside>
        <?php endif; ?>
...
<script>
  (() => {
    ...
    const requestedFileName = <?php echo json_encode($cta['contact_file_name'] ?? 'md-aminul-islam-itqan-consulting.vcf', 15, 512) ?>;
...
<?php echo $__env->make('frontend.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/laravel/ITQANConsulting/resources/views/frontend/pages/contact.blade.php ENDPATH**/ ?>